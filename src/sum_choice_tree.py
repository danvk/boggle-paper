...
class SumNode:
  ...
  def __init__(
    self, points: int = 0, children=None
  ):
    self.points = points
    self.children = children or []
    self.trie_node = None
...
def to_dot_help(
  ...
  if isinstance(node, ChoiceNode):
    for letter, child in sorted(
      node.children.items()
    ):
      child_id, child_dot = to_dot_help(
        child,
        cells,
        f"{me}{letter}",
        depth + 1,
      ...
      dot.append(
        f'{me} -- {child_id} [label="{letter.upper()}"]'
      )
  else:
    for i, child in enumerate(node.children):
      child_id, child_dot = to_dot_help(
        child, cells, f"{me}{i}", depth + 1
      )
      dot.append(child_dot)
